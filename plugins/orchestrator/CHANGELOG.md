...
- **플러그인 이름 변경**: `claude-devkit` → `orchestrator`
  - 에이전트 타입 네임스페이스 변경: `claude-devkit:*` → `orchestrator:*`
  - 세션 파일 경로 변경: `~/.claude-devkit-session-id` → `~/.orchestrator-session-id`
  - 설치 명령어 변경: `/plugin install orchestrator@jo-minjun`

### Breaking Changes

- **에이전트 타입 네임스페이스 변경**: 기존 `claude-devkit:code-explore`, `claude-devkit:planner` 등의 에이전트 타입이 `orchestrator:code-explore`, `orchestrator:planner` 등으로 변경됨
